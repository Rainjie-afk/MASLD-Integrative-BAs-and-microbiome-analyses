...
out_dir <- "NAFLD_BA_R_results_V5_4_MANUAL_COMPARISONS2"
n_permutations <- 9999
make_individual_boxplots <- TRUE
GROUP_LEVELS <- c("NC", "MCD", "3_5")
MANUAL_COMPARISONS <- data.frame(
  Comparison_ID = c("Comparison_1", "Comparison_2", "Comparison_3"),
  Numerator     = c("NC", "3_5", "3_5"),
  Denominator   = c("MCD",  "MCD", "NC"),
  stringsAsFactors = FALSE
)
MANUAL_COMPARISONS$Contrast <- paste0(
  MANUAL_COMPARISONS$Numerator, "_vs_", MANUAL_COMPARISONS$Denominator
)
if (nrow(MANUAL_COMPARISONS) != 3L) {
  stop("MANUAL_COMPARISONS must contain exactly three rows.")
}
if (anyNA(MANUAL_COMPARISONS$Numerator) ||
    anyNA(MANUAL_COMPARISONS$Denominator) ||
    any(MANUAL_COMPARISONS$Numerator == "") ||
    any(MANUAL_COMPARISONS$Denominator == "")) {
  stop("Every manual comparison must have a Numerator and Denominator.")
}
unknown_manual_groups <- setdiff(
  unique(c(MANUAL_COMPARISONS$Numerator, MANUAL_COMPARISONS$Denominator)),
  GROUP_LEVELS
)
if (length(unknown_manual_groups) > 0L) {
  stop(
    "Unknown group(s) in MANUAL_COMPARISONS: ",
    paste(unknown_manual_groups, collapse = ", "),
    ". Allowed groups are: ", paste(GROUP_LEVELS, collapse = ", ")
  )
}
if (any(MANUAL_COMPARISONS$Numerator == MANUAL_COMPARISONS$Denominator)) {
  stop("A manual comparison cannot compare a group with itself.")
}
if (anyDuplicated(MANUAL_COMPARISONS$Contrast) > 0L) {
  stop("MANUAL_COMPARISONS contains duplicated directed comparisons.")
}
...
purrr::walk(file.path(out_dir, subdirs), dir.create, showWarnings = FALSE, recursive = TRUE)
readr::write_csv(
  tibble::as_tibble(MANUAL_COMPARISONS),
  file.path(out_dir, "00_QC", "manual_group_comparisons.csv")
)
message(
  "Manual formal comparisons: ",
  paste(MANUAL_COMPARISONS$Contrast, collapse = "; ")
)
...
make_emmeans_contrast_list <- function(
    group_levels = GROUP_LEVELS,
    comparisons = MANUAL_COMPARISONS) {
  methods <- lapply(seq_len(nrow(comparisons)), function(i) {
    v <- rep(0, length(group_levels))
    names(v) <- group_levels
    v[comparisons$Numerator[i]] <- 1
    v[comparisons$Denominator[i]] <- -1
    unname(v)
  })
  names(methods) <- comparisons$Contrast
  methods
}
EMMEANS_CONTRASTS <- make_emmeans_contrast_list()
...
if (!file.exists(metadata_file)) {
  metadata_template <- data %>%
    dplyr::select(Sample_ID, Group, Part) %>%
    mutate(
...
data <- data %>%
  dplyr::select(-any_of(c("Treatment_detail", "Animal_ID", "Replicate_Set", "Batch", "Notes"))) %>%
  left_join(
    metadata %>%
      dplyr::select(Sample_ID, Group, Part, Treatment_detail,
  ...
  mutate(
    Group = factor(Group_metadata, levels = GROUP_LEVELS),
  ...
  dplyr::select(-Group_metadata, -Part_metadata)
...
sample_qc <- data %>%
  ...
  dplyr::select(Sample_ID, Group, Part, Treatment_detail, Animal_ID, Batch,
...
kruskal_results <- all_long %>%
  ...
  nest() %>%
  mutate(test = purrr::map(data, safe_kruskal)) %>%
  dplyr::select(-data) %>%
  unnest(test) %>%
  ungroup() %>%
  group_by(Part) %>%
...
contrasts <- tibble::tibble(
  Group1 = MANUAL_COMPARISONS$Denominator,
  Group2 = MANUAL_COMPARISONS$Numerator,
  Contrast = MANUAL_COMPARISONS$Contrast
)
...
fit_log2_lm <- function(data, feature_name) {
  ...
  manual_contrasts <- function(fit_object) {
    model_coef <- stats::coef(fit_object)
    vc <- stats::vcov(fit_object)
    if (length(model_coef) == 0L || anyNA(model_coef)) return(NULL)
    if (!all(names(model_coef) %in% rownames(vc)) ||
        !all(names(model_coef) %in% colnames(vc))) {
      return(NULL)
    }
    vc <- vc[names(model_coef), names(model_coef), drop = FALSE]
    group_model_row <- function(group_name) {
      nd <- data.frame(Group = factor(group_name, levels = GROUP_LEVELS))
      mm <- stats::model.matrix(~ Group, data = nd)
      out <- stats::setNames(rep(0, length(model_coef)), names(model_coef))
      common <- intersect(colnames(mm), names(out))
      out[common] <- mm[1, common]
      out
    }
    purrr::map_dfr(seq_len(nrow(MANUAL_COMPARISONS)), function(i) {
      numerator <- MANUAL_COMPARISONS$Numerator[i]
      denominator <- MANUAL_COMPARISONS$Denominator[i]
      contrast_name <- MANUAL_COMPARISONS$Contrast[i]
      cvec <- group_model_row(numerator) - group_model_row(denominator)
      estimate <- sum(cvec * model_coef)
      variance <- as.numeric(t(cvec) %*% vc %*% cvec)
      if (!is.finite(variance) || variance < 0) {
        return(tibble::tibble(
          Contrast = contrast_name,
          estimate_log2FC = estimate,
          SE = NA_real_,
          df = stats::df.residual(fit_object),
          statistic = NA_real_,
          p_value = NA_real_
        ))
      }
...
lm_results <- all_long %>%
  group_by(Part, Feature_type, Feature) %>%
  nest() %>%
  mutate(result = purrr::map2(data, Feature, fit_log2_lm)) %>%
  dplyr::select(-data) %>%
  unnest(result) %>%
  ungroup() %>%
  group_by(Part, Contrast) %>%
...
median_profiles <- all_long %>%
  ...
  nest() %>%
  mutate(
    profile = purrr::map2(data, Feature, summarise_feature_profile)
  ) %>%
  dplyr::select(-data) %>%
  unnest(profile) %>%
  pivot_wider(
...
make_effect_heatmap <- function(feature_type, output_name) {
  ...
  effect_df <- heat %>%
    dplyr::select(Feature, Column, log2FC) %>%
    pivot_wider(
      id_cols = Feature,
      names_from = Column,
      values_from = log2FC
    ...
    arrange(Feature)
  symbol_df <- heat %>%
    dplyr::select(Feature, Column, q_symbol) %>%
    pivot_wider(
      id_cols = Feature,
      names_from = Column,
...
run_absolute_multivariate <- function(part_name) {
  ...
  scores <- as_tibble(pca$x[, 1:min(3, ncol(pca$x)), drop = FALSE],
                      rownames = "Sample_ID") %>%
    left_join(part_data %>% dplyr::select(Sample_ID, Group, Treatment_detail), by = "Sample_ID")
  ...
  meta <- part_data %>% dplyr::select(Group, Treatment_detail)
...
make_centroid_trajectory <- function(part_name) {
  ...
  scores <- as_tibble(pca$x[, 1:2, drop = FALSE]) %>%
    bind_cols(part_data %>% dplyr::select(Group))
  centroids <- scores %>%
    group_by(Group) %>%
    summarise(PC1 = mean(PC1), PC2 = mean(PC2), .groups = "drop") %>%
    mutate(Group_order = match(Group, GROUP_LEVELS)) %>%
    arrange(Group_order)
...
purrr::walk(levels(droplevels(analysis_data$Part)), make_centroid_trajectory)
...
run_residual_correlation <- function(part_name) {
  part_data <- analysis_data %>% filter(Part == part_name)
  x <- part_data %>% dplyr::select(all_of(ba_columns))
...
purrr::walk(levels(droplevels(analysis_data$Part)), run_residual_correlation)
...
if (run_plasma_detail_analysis) {
  ...
  plasma_sensitivity_long <- plasma_sensitivity %>%
    dplyr::select(Sample_ID, Plasma_group, all_of(ba_columns), all_of(pathway_features)) %>%
    pivot_longer(
...
if (has_animal_id) {
  ...
  if (requireNamespace("lme4", quietly = TRUE) &&
    ...
    fit_mixed_one_feature <- function(feature_name) {
      model_data <- matched_data %>%
        dplyr::select(Animal_ID, Group, Part, Batch, all_of(feature_name)) %>%
  ...
  if (requireNamespace("glmmTMB", quietly = TRUE) &&
    ...
    fit_beta_mixed <- function(feature_name) {
      d <- matched_data %>%
        dplyr::select(Animal_ID, Group, Part, all_of(feature_name)) %>%
  ...
  matched_index_long <- matched_data %>%
    dplyr::select(Animal_ID, Group, Part, all_of(c(ba_columns, cross_tissue_features))) %>%
    pivot_longer(
  ...
  cross_tissue_correlations <- map_dfr(part_pairs, function(parts_pair) {
    ...
    wide <- matched_index_long %>%
      ...
      dplyr::select(Animal_ID, Group, Part_feature, Value) %>%
  ...
  if (requireNamespace("mixOmics", quietly = TRUE)) {
    ...
    if (can_run_diablo) {
      ...
      y <- droplevels(factor(y_table$Group, levels = GROUP_LEVELS))
      block_names <- required_parts
      removed_diablo_features <- list()
      blocks <- purrr::set_names(purrr::map(block_names, function(part_name) {
        d <- complete_data %>%
          filter(Part == part_name) %>%
